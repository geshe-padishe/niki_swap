...
/*   Updated: 2020/12/16 00:57:13 by nikotikch        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	if (!(dynarray->list = malloc(dynarray->byte_size)))
		return (-1);
	if (!(dynarray->tmp = malloc(dynarray->byte_size)))
		return (-1);
	if (nb_cells == 0)
...
int push_dynarray(t_dynarray *darr, void *new_cells, uint64_t nb_ncells, bool way)
{
	uint64_t	nb_cells;
	uint64_t	new_size;
